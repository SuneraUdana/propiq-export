...
def _compute_features(r: dict, suburb_median: float) -> dict:
    ...
    land  = max(r.get("land_size_sqm") or 200, 1)
    ...
    if isinstance(nlp_raw, str):
        try: nlp_raw = json.loads(nlp_raw)
        except: nlp_raw = {}

    yield_proxy = max(0, (suburb_median - price) / suburb_median)
    age_risk    = max(0, (2024 - year) / 130)
    price_risk  = min(1, price / 3_000_000)
    neg_nlp     = float(nlp_raw.get("needs_work",0) or nlp_raw.get("sentiment_neg",0))
    ...
    return {"yield_proxy": yield_proxy, "risk_score": risk_score,
...
def score_and_rank(records: list[dict], weights: np.ndarray = None,
                   verbose: bool = False) -> list[dict]:
    suburb_prices = defaultdict(list)
    for r in records:
        if r.get("sale_price"): suburb_prices[r["suburb"]].append(r["sale_price"])
    suburb_med = {s: np.median(v) for s, v in suburb_prices.items()}

    if weights is None:
        all_feats = [_compute_features(r, suburb_med.get(r["suburb"],1_000_000)) for r in records]
        weights   = optimise_weights(all_feats, verbose=verbose)

    w0, w1, w2, w3 = weights
    wj = weights.tolist()

    suburb_scores = defaultdict(list)
    scored = []
    for r in records:
        f = _compute_features(r, suburb_med.get(r["suburb"], 1_000_000))
        ...
        scored.append(row)
        suburb_scores[r["suburb"]].append(row)

    for sub_recs in suburb_scores.values():
        for rank, rec in enumerate(sorted(sub_recs, key=lambda x: x["inv_score"], reverse=True), 1):
